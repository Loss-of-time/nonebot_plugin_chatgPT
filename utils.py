...
async def call_api(model: Model, messages: List[dict], api_key: str, api_url: str, max_tokens: int, logger: "Logger") -> tuple[int, str]:
    async with aiohttp.ClientSession() as session:
        headers = {
        ...
        try:
            async with session.post(f"{api_url}/chat/completions", json=data, headers=headers) as resp:
                try:
